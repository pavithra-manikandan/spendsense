...
import { TrendingUp, TrendingDown, DollarSign, AlertTriangle, Repeat, Zap } from "lucide-react";
import { motion } from "framer-motion";
import { getAnalysis } from "@/lib/api";

const CAT_COLORS: Record<string, string> = {
  food: "#f0a030", groceries: "#40c9a2", shopping: "#9b7dff", transport: "#5ba4f5",
  subscriptions: "#e85d50", health: "#f06090", entertainment: "#c084fc",
...
export function Dashboard({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  if (!analysis || !analysis.transaction_count) {
    return (
      ...
      >
        ...
        <p className="text-muted-foreground mb-6">Upload a bank CSV to activate your spendsense</p>
        <button
  ...
  const pieConfig: ChartConfig = Object.fromEntries(
    Object.entries(analysis.by_category).map(([cat]) => [
      cat, { label: `${CAT_EMOJI[cat] || "📦"} ${cat.charAt(0).toUpperCase() + cat.slice(1)}`, color: CAT_COLORS[cat] || "#6b7280" }
    ])
  );
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              {pieData.map(d => (
                <span key={d.category} className="text-xs text-muted-foreground flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-sm inline-block" style={{ background: CAT_COLORS[d.category] || "#6b7280" }} />
...
function BarChartComponent({ data }: { data: { merchant: string; amount: number; count: number }[] }) {
  return (
    <BarChart data={data} layout="vertical" width={450} height={280} margin={{ left: 5, right: 20 }}>
      <XAxis type="number" tick={{ fill: "#8a8880", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => `$${v}`} />
      <YAxis type="category" dataKey="merchant" tick={{ fill: "#8a8880", fontSize: 11 }} axisLine={false} tickLine={false} width={100} />
      <Tooltip
...
function TrendChartComponent({ data }: { data: { month: string; total: number }[] }) {
  return (
    <LineChart data={data} width={900} height={200} margin={{ left: 10, right: 20, top: 10 }}>
      ...
      <XAxis dataKey="month" tick={{ fill: "#8a8880", fontSize: 11 }} axisLine={false} />
      <YAxis tick={{ fill: "#8a8880", fontSize: 11 }} axisLine={false} tickFormatter={v => `$${v}`} />
      <Tooltip
